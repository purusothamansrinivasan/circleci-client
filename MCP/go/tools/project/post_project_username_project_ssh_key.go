...
func Post_project_username_project_ssh_keyHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.Projectusernameprojectsshkeypostrequest
		...
		url := fmt.Sprintf("%s/project/%s/%s/ssh-key%s", cfg.BaseURL, username, project, queryString)
		...
		var result models.Projectusernameprojectsshkeypostdefaultresponse
...
func CreatePost_project_username_project_ssh_keyTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_project_username_project_ssh-key",
		mcp.WithDescription("Create an ssh key used to access external systems that require SSH key-based authentication
"),
		...
		mcp.WithString("private_key", mcp.Description("")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Post_project_username_project_ssh_keyHandler(cfg),
	}
}
