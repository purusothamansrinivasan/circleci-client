...
func Delete_project_username_project_build_cacheHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/project/%s/%s/build-cache%s", cfg.BaseURL, username, project, queryString)
		...
		var result models.Projectusernameprojectbuildcachedelete200response
...
func CreateDelete_project_username_project_build_cacheTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_project_username_project_build-cache",
		mcp.WithDescription("Clears the cache for a project.
"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Delete_project_username_project_build_cacheHandler(cfg),
	}
}
