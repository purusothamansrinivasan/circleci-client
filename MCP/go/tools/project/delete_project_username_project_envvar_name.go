...
func Delete_project_username_project_envvar_nameHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/project/%s/%s/envvar/%s%s", cfg.BaseURL, username, project, name, queryString)
...
func CreateDelete_project_username_project_envvar_nameTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_project_username_project_envvar_name",
		mcp.WithDescription("Deletes the environment variable named ':name'
"),
		...
		mcp.WithString("name", mcp.Required(), mcp.Description("XXXXXXXXXX\n")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Delete_project_username_project_envvar_nameHandler(cfg),
	}
}
