/**
 * MCP Server function for Full details for a single build. The response includes all of the fields from the build summary.
...
class Get_Project_Username_Project_Build_NumMCPTool {
    
    public static Function<MCPServer.MCPRequest, MCPServer.MCPToolResult> getGet_Project_Username_Project_Build_NumHandler(MCPServer.APIConfig config) {
        return (request) -> {
            try {
        ...
        if (args.containsKey("build_num")) {
            queryParams.add("build_num=" + args.get("build_num"));
                ...
                String url = config.getBaseUrl() + "/api/v2/get_project_username_project_build_num" + queryString;
    ...
    public static MCPServer.Tool createGet_Project_Username_Project_Build_NumTool(MCPServer.APIConfig config) {
...
This is also the payload for the [notification webhooks](/docs/configuration/#notify), in which case this object is the value to a key named 'payload'.",
        ...
        return new MCPServer.Tool(definition, getGet_Project_Username_Project_Build_NumHandler(config));
    }
